...
import org.processmining.plugins.tracealignmentwithguidetree.util.ArrayMatrixFileIO;
...
public class ConsensusSequence extends Sequence {
	...
	int[] predominantActivityIndexGapsNotAllowed;
	...
	public void recalc() {
		...
		predominantActivityIndexGapsNotAllowed = new int[noColumns];
		for (int k = noColumns - 1; k >= 0; k--) {
			...
			for (int i = activityFrequencyInColumnTable[k].length - 1; i >= 0; i--) {
				if (activityFrequencyInColumnTable[k][i] >= maxCount) {
					...
					if (!activityList.get(i).equals(dash)) predominantActivityIndexGapsNotAllowed[k] = i;  // predominant activity sequence with gaps not allowed
				}
			}
		}
	...
	public double getSimplePercentOccurrence(int index, boolean ignoreGaps) {
		int integerLabelOfPredominantActivity;
		if (ignoreGaps)
			integerLabelOfPredominantActivity = predominantActivityIndexGapsNotAllowed[index];
		else
			integerLabelOfPredominantActivity = predominantActivityIndex[index];
		int frequency = activityFrequencyInColumnTable[index][integerLabelOfPredominantActivity];
		int indexMax = sequencesInAlignment.length;
		double simplePercentOccurrence = (double) frequency/indexMax;
		if (simplePercentOccurrence > 1) simplePercentOccurrence = 1;
		if (simplePercentOccurrence < 0) simplePercentOccurrence = 0;
		return simplePercentOccurrence;
	}

	public void printActivityFrequencyInColumnTableToFile(String outputFileName) {
		ArrayMatrixFileIO fileIO = new ArrayMatrixFileIO();
		String outputDir = System.getProperty("java.io.tmpdir")+"\\TraceAlignmentWithGuideTree";
		fileIO.writeToFile(outputDir, outputFileName, activityFrequencyInColumnTable);
	}
}

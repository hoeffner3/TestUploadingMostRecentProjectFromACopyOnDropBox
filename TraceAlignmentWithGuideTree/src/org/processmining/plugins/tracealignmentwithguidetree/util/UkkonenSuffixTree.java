/*
 * Author: R. P. Jagadeesh Chandra Bose Date: 22 Dec 2008 Version: 1.0
 * 
 * This file implements the suffix tree generation in linear time and space The
 * suffix tree is constructed based on Ukkonen's algorithm The general outline
 * of the algorithm is as follows:
 * 
 * n = length of the string. CreateTree: Calls n times to SPA (Single Phase
 * Algorithm). SPA: Increase the variable e (virtual end of all leaves). Calls
 * SEA (Single Extension Algorithm) starting with the first extension that does
 * not already exist in the tree and ending at the first extension that already
 * exists. SEA : Follow suffix link. Check if current suffix exists in the tree.
 * If it does not - apply rule 2 and then create a new suffix link.
 * apply_rule_2: Create a new leaf and maybe a new internal node as well.
...
final class SuffixNode {
	...
	public SuffixNode() {
		...
		suffixLink = null;
		firstChild = null;
		childrenList = new ArrayList<SuffixNode>();
		edgeLabelStart = 0;
		edgeLabelEnd = 0;
		pathPosition = 0;
	}
...
public class UkkonenSuffixTree {
	...
	public UkkonenSuffixTree(int encodingLength, String sequence) {
		 ...
		 */
		sequenceLength = sequence.length() / encodingLength + 1;
	...
	private int SEA(SuffixTreePath streePath, boolean afterRule3) {
		...
		if (pos.node == root) {
			...
			if (isLastSymbolInEdge()) {
				tempNode = findChild(sequence.substring(streePath.end * encodingLength, (streePath.end + 1)
						* encodingLength));
				if (tempNode != null) {
					pos.node = tempNode;
					pos.edgePosition = 0;
				...
				String str1 = sequence.substring(tempIndex * encodingLength, (tempIndex + 1) * encodingLength);
				String str2 = sequence.substring(streePath.end * encodingLength, (streePath.end + 1) * encodingLength);
				if (str1.equals(str2)) {
					pos.edgePosition++;
					noSymbolsFound = 1;
				}
	 ...
	 * 
	 */
	private void applyExtensionRule2(int edgeLabelBegin, int edgeLabelEnd, int pathPosition, int edgePos, boolean newson) {
		...
		SuffixNode newInternal = new SuffixNode(pos.node.parent, pos.node.edgeLabelStart, pos.node.edgeLabelStart
				+ edgePos, pos.node.pathPosition);
	...
	private int traceString(SuffixTreePath streePath, String searchString, boolean skip) {
		/*
		 * This variable will be true when search is done. It is a return value
		 * from function traceSingleEdge
		 */
	...
	private TraceSingleEdgeResult traceSingleEdge(SuffixTreePath streePath, boolean skip) {
		...
		SuffixNode contNode = findChild(sequence.substring(streePath.begin * encodingLength, (streePath.begin + 1)
				* encodingLength));
		...
		if (skip) {
			...
			for (edgePos = 1, noEdgeSymbolsFound = 1; edgePos < nodeLabelLength; edgePos++, noEdgeSymbolsFound++) {
				 ...
				 * equal - continue
				 */
				if (!sequence.substring((pos.node.edgeLabelStart + edgePos) * encodingLength,
						(pos.node.edgeLabelStart + edgePos + 1) * encodingLength).equals(
						sequence.substring((streePath.begin + edgePos) * encodingLength,
								(streePath.begin + edgePos + 1) * encodingLength))) {
					edgePos--;
	...
	private TraceSingleEdgeResult traceSingleEdge(SuffixTreePath streePath, String searchString, boolean skip) {
		...
		SuffixNode contNode = findChild(searchString.substring(streePath.begin * encodingLength, (streePath.begin + 1)
				* encodingLength));
		...
		if (skip) {
			...
			for (edgePos = 1, noEdgeSymbolsFound = 1; edgePos < nodeLabelLength; edgePos++, noEdgeSymbolsFound++) {
				...
				if (!sequence.substring((pos.node.edgeLabelStart + edgePos) * encodingLength,
						(pos.node.edgeLabelStart + edgePos + 1) * encodingLength).equals(
						searchString.substring((streePath.begin + edgePos) * encodingLength,
								(streePath.begin + edgePos + 1) * encodingLength))) {
					edgePos--;
					pos.edgePosition = edgePos;
					// Logger.printReturn("Exiting traceSingleEdge");
	...
	private SuffixNode findChild(String symbol) {
		...
		for (i = 0; i < noChildren; i++) {
			if (sequence.substring(pos.node.childrenList.get(i).edgeLabelStart * encodingLength,
					(pos.node.childrenList.get(i).edgeLabelStart + 1) * encodingLength).equals(symbol)) {
				break;
			}
		}
	...
	private void printNode(SuffixNode node, int depth) {
		...
		if (depth > 0) {
			...
			while (start <= end) {
				System.out.print(sequence.substring(start * encodingLength, (start + 1) * encodingLength));
				start++;
			}

			System.out.print("(" + node.isLeftDiverse + "  " + node.leftSymbol + "  " + node.pathPosition + " : "
					+ node.edgeLabelStart + "," + end + ")");
			System.out.println();
		}
	...
	public void findLeftDiverseNodes() {
		 ...
		 */
		for (SuffixNode leafNode : leaves) {
			leafNode.leftSymbol = sequence.substring((leafNode.pathPosition - 1) * encodingLength,
	...
	public HashSet<String> getMaximalRepeats() {
		...
		String repeat;
		//		System.out.println("AdjustedPrefix = "+adjustedPrefix);
		for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
			repeat = sequence.substring(leftDiverseNode.pathPosition * encodingLength,
					(leftDiverseNode.edgeLabelEnd + 1) * encodingLength);
			if (!repeat.equals(adjustedPrefix)) {
				maximalRepeats.add(repeat);
			}
		}
		...
		for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
			...
			if (isAllChildrenLeaves && (leftSymbolSet.size() == leftDiverseNode.childrenList.size())) {
				repeat = sequence.substring(leftDiverseNode.pathPosition * encodingLength,
						(leftDiverseNode.edgeLabelEnd + 1) * encodingLength);
				if (!repeat.equals(adjustedPrefix)) {
					superMaximalRepeats.add(repeat);
				}
			}
		}
		...
		for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
			...
			if (isNearSuperMaximal) {
				repeat = sequence.substring(leftDiverseNode.pathPosition * encodingLength,
						(leftDiverseNode.edgeLabelEnd + 1) * encodingLength);
				if (!repeat.equals(adjustedPrefix)) {
					nearSuperMaximalRepeats.add(repeat);
				}
			}
		}
	...
	public void LZDecomposition() {
		...
		for (int i = 2; i <= sequenceLength; i++) {
			...
			currentSymbol = sequence.substring(i * encodingLength, (i + 1) * encodingLength);

			pos.node = root;
			SuffixNode childNode = findChild(currentSymbol);
			if (childNode == null) {
				System.out.println("Something terribly Wrong; Current Symbol " + currentSymbol + " not found");
				System.exit(0);
			}
			...
			while (++i < sequenceLength) {
				nextSymbol = sequence.substring(i * encodingLength, (i + 1) * encodingLength);
				...
				if (childNode.edgeLabelStart < i) {
					...
					noMatches += childNode.edgeLabelEnd - childNode.edgeLabelStart + 1;
		...
		for (int i = 0; i < noBlocks; i++) {
			blocks.add(sequence.substring(I.get(i) * encodingLength, I.get(i + 1) * encodingLength));
			h[i] = I.get(i);
		}
	...
	private void processBlockAlgorithm1A(int currentBlockLength, int h1) {
		...
		for (int k = 1; k <= currentBlockLength; k++) {
			...
			while ((h1 + k1 + 1 < sequenceLength)
					&& sequence.substring((q + k1) * encodingLength, (q + k1 + 1) * encodingLength).equals(
							sequence.substring((h1 + k1) * encodingLength, (h1 + k1 + 1) * encodingLength))) {
				k1++;
			}
			...
			k2 = 0;
			while (((q - k2 - 1) > 0)
					&& sequence.substring((q - 1 - k2) * encodingLength, (q - k2) * encodingLength).equals(
							sequence.substring((h1 - k2 - 1) * encodingLength, (h1 - k2) * encodingLength))) {
				k2++;
			}
	...
	private void processBlockAlgorithm1B(int currentBlockLength, int nextBlockLength, int h, int h1) {
		...
		for (int k = 1; k <= currentBlockLength + nextBlockLength; k++) {
			...
			while (((q + k1) < sequenceLength)
					&& sequence.substring((h + k1) * encodingLength, (h + k1 + 1) * encodingLength).equals(
							sequence.substring((q + k1) * encodingLength, (q + k1 + 1) * encodingLength))) {
				k1++;
			}
			...
			k2 = 0;
			while (((h - k2 - 1) > 0)
					&& sequence.substring((h - k2 - 1) * encodingLength, (h - k2) * encodingLength).equals(
							sequence.substring((q - 1 - k2) * encodingLength, (q - k2) * encodingLength))) {
				k2++;
			}

			if ((k1 + k2 >= k) && (k1 > 0) && (k2 > 0) && ((max(h - k2, h - k + 1) + k) <= h1)) {
		...
		for (int i = 0; i < sequenceLength; i++) {
			if (tandemPairs[i].size() > 0) {
				for (Integer loc : tandemPairs[i]) {
					tandemRepeat = sequence.substring((i + 1) * encodingLength, (i + 1 + loc) * encodingLength);
			...
			if (!isTandemRepeatProcessed[index]) {
				 ...
				 */else {
					...
					if (found) {
						...
						complexTandemRepeats.add(tr.substring(0 * encodingLength, tandemRepeatLength * encodingLength
								/ 2));
					} else {
	...
	public static void main(String[] args) {
		...
		System.out.println("Took " + w.msecs() + " msecs to construct the tree");
		...
		System.out.println("Took " + w.msecs() + " msecs to find the leftDiversity of nodes");
		...
		System.out.println("Took " + w.msecs() + " msecs to find the maximal repeats in the string");
	...
	public void singleStringTest() {
		...
		for (int i = 0; i < stringLength; i++) {
			symbol = str.substring(i * encodingLength, (i + 1) * encodingLength);
			pos.node = st.root;
